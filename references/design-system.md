...
For tinted overlays, borders, and hover states, use the `opacity` property (0.0-1.0). Below are the **most commonly used stops** - for the full 0%/2%/4%/8%/15%/20%/25%/50%/70%/80%/90%/95% scale see the original token source.

##### Opacity Black

| Opacity | Value | Typical Use |
|---------|-------|-------------|
| 0% | `0a0a0a00` | Fully transparent |
| 8% | `0a0a0a14` | Border / divider |
| 50% | `0a0a0a80` | Semi-transparent overlay |
...
| 8% | `ffffff14` | Border / divider |
| 50% | `ffffff80` | Semi-transparent overlay |
...
Default PptxGenJS charts look dated. Apply this recipe for a modern, clean appearance that matches the rest of the deck.
...
|--------|---------|-------|
| `chartColors: [...]` | Series / segment colors | Pull from palette (see Agent Design System -> Blue Scale) |
| `chartArea: { fill, border, roundedCorners }` | Chart background | White + roundedCorners = modern |
| `catAxisLabelColor / valAxisLabelColor` | Axis label color | Muted gray (`64748B`) |
| `valGridLine / catGridLine` | Grid lines | Show value-axis only; set `style: "none"` to hide |
| `lineSmooth: true` | Curved line chart | Line charts only |
| `showValue + dataLabelPosition` | On-bar labels | `"outEnd"` for bars |
| `legendPos: "b"\|"t"\|"l"\|"r"\|"tr"` | Legend placement | Hide for single-series (`showLegend: false`) |

### Constraints

- Always source `chartColors` from the chosen palette - never invent hex values
- Hide `catGridLine` (`style: "none"`); keep only `valGridLine` for readability
- For single-series charts: `showLegend: false` to avoid visual noise
- Match axis label color to body text (use `theme.secondary`, not black)

See [pptxgenjs.md](pptxgenjs.md) "Charts" for the `addChart` API surface; see [chart-schemas.md](chart-schemas.md) for the data shape.
